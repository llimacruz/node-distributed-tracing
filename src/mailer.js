...
const topicName = 'emails';
const tracer = tracingUtil.init("mailer");

(async () => {
  try {
    const kafka = new Kafka({ clientId: 'mailer', brokers: ['localhost:9092'] });
    const consumer = await kafkaUtil.subscribeTopics(kafka, 'mailer', ['receipts', 'deliveries']);
    ...
    await consumer.run({
      eachMessage: async ({ message, topic }) => {
        ...
        console.log(`Parent (${topic === 'receipts' ? 'Receipt' : 'Delivery'}) spanId: ${msg.spanData.spanIdStr}`);
